  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f0fdf4', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ background: 'white', padding: '2rem', borderRadius: '12px', boxShadow: '0 4px 20px rgba(0,0,0,0.08)', width: '100%', maxWidth: '400px' }}>
        <h1 style={{ color: '#166534', marginBottom: '0.25rem', fontSize: '1.5rem', fontWeight: 700 }}>FarmStock AI</h1>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <button type="submit" disabled={loading}
            style={{ width: '100%', background: '#15803d', color: 'white', padding: '0.625rem', borderRadius: '8px', fontWeight: 600, border: 'none', cursor: loading ? 'not-allowed' : 'pointer', opacity: loading ? 0.7 : 1 }}>
            {loading ? 'Signing in...' : 'Sign in'}
          </button>
        </form>
        <p style={{ marginTop: '1rem', fontSize: '0.875rem', textAlign: 'center', color: '#6b7280' }}>
          No account?{' '}
          <Link to="/register" style={{ color: '#15803d', fontWeight: 500 }}>Start free 14-day trial</Link>
        </p>
      </div>
    </div>
  )
